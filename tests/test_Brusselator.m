%% Test for Brusselator matrix

    %% Matrix structure and eigenvalues
    n=800;
    r=2;
    rr=20;
    A=load('brusselator.txt');
    A=sparse(A(2:end,1),A(2:end,2),A(2:end,3),n,n);
    v=sort(eigs(A,rr,'largestreal'),'descend','ComparisonMethod','real');
    [row,col]=find(A);
    ...
    eig_tot{j}=eigs(A+Dist(j)*E,rr,'largestreal');
    ...
    E=projsparse(row,col,U,S,V);
    k=2;
    eig_tot{k}=eigs(A+Dist(j)*E,rr,'largestreal');
